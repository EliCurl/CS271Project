...
public class Board extends JPanel {
    ...
    private static final boolean DISCARD_BY_RATIO = true; // If true, discard splits with extreme ratios
    private static final double H_RATIO = 0.45;        // Minimum acceptable height ratio (for horizontal splits)
    private static final double W_RATIO = 0.45;        // Minimum acceptable width ratio (for vertical splits)
    ...
    public static class Container {
        ...
        public void drawPath(Graphics2D g, Container other) {
            ...
            g.drawLine(center.x * SQUARE, center.y * SQUARE, other.center.x * SQUARE, other.center.y * SQUARE);
        }
    }
    ...
    // Room is generated within a container, with random padding to create a smaller room.
    public static class Room {
        int x, y, w, h;
        
        public Room(Container container) {
            // Apply random padding up to 1/3 of the container's width/height.
            int padX = randomGenerator.nextInt(Math.max(1, container.w / 3 + 1));
            int padY = randomGenerator.nextInt(Math.max(1, container.h / 3 + 1));
            this.x = container.x + padX;
            this.y = container.y + padY;
            this.w = container.w - padX;
            this.h = container.h - padY;
            
            // Subtract a random value to further shrink the room dimensions.
            int reduceW = randomGenerator.nextInt(Math.max(1, this.w / 3 + 1));
            int reduceH = randomGenerator.nextInt(Math.max(1, this.h / 3 + 1));
            this.w = Math.max(1, this.w - reduceW);
            this.h = Math.max(1, this.h - reduceH);
        }
        
        // Fill the room as a gray rectangle.
        public void draw(Graphics2D g) {
    ...
    // Generate the dungeon: split the main container and create rooms in the leaf containers.
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Optionally enable anti-aliasing for smoother lines
        ...
        // Draw the rooms in the leaf containers.
        for (Room room : rooms) {
            room.draw(g2d);
        }
    }
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("BSP Dungeon Generation");
